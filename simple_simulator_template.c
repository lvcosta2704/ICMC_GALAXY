...
#define CMP 22
#define SWAP 23 // "010110"; -- CMP Rx Ry  		-- Compare Rx and Ry and set FR :  FL = <...|over|carry|zero|equal|lesser|greater>	  Format: < inst(6) | Rx(3) | Ry(3) | xxxx >
...
loop:
	...
	case STATE_DECODE:
		...
		case CALL:
			...
			if ((COND == 0)										// NO COND
				...
				|| (FR[9] == 1 && (COND == 13)))				// DIVBYZERO
			{													// PC = MEMORY[PC];
				selM1 = sSP;									// mux 1 endereça com o stack pointer
				RW = 1;
				selM5 = sPC; // deixa passar o PC
				DecSP = 1;
				state = STATE_EXECUTE;
			}
			else
		...
		case PUSH:
			selM3 = rx;
			selM5 = sM3; // passa o reg 3 pelo mux 5
			RW = 1;		 // escrita na memoria
			selM1 = sSP;
			DecSP = 1; // empilha o conteudo do registrador
			// -----------------------------
			state = STATE_FETCH;
			break;
	...
	case STATE_EXECUTE:
		...
		case CALL:
			selM1 = sPC;
			RW = 0;
			LoadPC = 1; // poe o numero que queremos jumpar no PC
			// -----------------------------
			state = STATE_FETCH;
			break;

		case POP:
			selM1 = sSP;	   // endereça a memoria com o SP
			RW = 0;			   // poe a memoria em leitura
			selM2 = sDATA_OUT; // seleciona com o mux 2 para passar para os registradores
			LoadReg[rx] = 1;   // entra dentro do registrador a informacao do pop
			// -----------------------------
			state = STATE_FETCH;
			break;
	...
	// Selecao do Mux6
	if (selM6 == sULA)
		M6 = resultadoUla.auxFR; // TODO: Talvez o auxFR deva ser o valor do FR //**Sempre recebe flags da ULA
...
void le_arquivo(void)
{
	...
	if ((stream = fopen("cpuram4.mif", "r")) == NULL) // Abre o arquivo para leitura
	{
	...
	while (fscanf(stream, "%s", linha) != EOF) // Le linha por linha ate' o final do arquivo: eof = end of file !!
	{
		...
		if (!processando)
		{
			i = 0;
			do
			{ // Busca por sequencias de caracteres para catar inicio do codigo
				letra[0] = letra[1];
				letra[1] = linha[i];
...
ResultadoUla ULA(unsigned int x, unsigned int y, unsigned int OP, int carry)
{
	...
	case LOGIC:
		...
		else
		{
			...
			case SWAP:
				// isola o mais baixo dos bits e move pra cima
				unsigned int low_byte = (x & 0x00FF) << 8;
				// isola o bit alto e joga pra baixo
				unsigned int high_byte = (x & 0xFF00) >> 8;
				// combina os dois bits com um OR
